...
The run needs no server or account: it goes through demo mode. Regenerate when the startup path
changes shape rather than on every commit, and check the output before committing it — two failure
modes produce a plausible-looking but worthless profile. Runbook:
